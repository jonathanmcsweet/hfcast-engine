...
C     PROPCORE_TRACE names a directory.
     ...
     C ,YMUF(4)
      ...
      write(90,'(8E18.9,2I4)') EMUF(IT),F1MUF(IT),F2MUF(IT),ESMUF(IT),
     A ALLMUF(IT),FOT(IT),HPF(IT),ANGMUF(IT),MODMUF,KS
      do 900 L = 1, 4
      write(90,'(A,7E18.9,I8,3E18.9)') 'PT ',SIGL(L),SIGU(L),DELMUF(L),
     A HPMUF(L),HTMUF(L),FVMUF(L),AFMUF(L),NHOPMF(L),YFOT(L),YMUF(L),
     B YHPF(L)
900   continue
      close(90)
      RETURN
