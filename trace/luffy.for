...
ccc     +        ifx,angle(ifx),dblos(ifx)
      IF(IPFG.LT.300) call trace_son(IFX,IPFG)
...
C     after ESREG (IHOPX = 99) when PROPCORE_TRACE names a directory.
     ...
     3 OBF(7),NMODE(7),TLLOW(7),TLHGH(7)
      ...
      do 10 i=1,7
      write(84,'(5E18.9)') ABPS(i),CREL(i),EFF(i),FLDST(i),GRLOS(i),
     A HN(i),HP(i),PROB(i),RELY(i),RGAIN(i),SIGPOW(i),SN(i),SPRO(i),
     B TGAIN(i),TIMED(i),TLOSS(i),B(i),FSLOS(i),ADV(i),OBF(i),
     C FLOAT(NMODE(i)),TLLOW(i),TLHGH(i)
   10 continue
      close(84)
      RETURN
      END
...
C     final ALLMODES call (short path) or LNGPAT (long path).
     ...
     CizNMODE(20),zTLLOW(20),zTLHGH(20),zEFF(20),NREL,NMMOD
      ...
      write(83,'(A,I4,I8,I4,I4)') 'AMD ',NINT(GMT),NINT(FREQ*1000.),
     A NMMOD,NDMP
      do 10 i=1,NDMP
      write(83,'(5E18.9)') zABPS(i),zCREL(i),zEFF(i),zFLDST(i),
     A zgrlos(i),zHN(i),zHP(i),zPROB(i),zRELY(i),zRGAIN(i),zSIGPOW(i),
     B zSN(i),zSPRO(i),zTGAIN(i),zTIMED(i),zTLOSS(i),zB(i),zFSLOS(i),
     C zadv(i),zobf(i),FLOAT(izNMODE(i)),zTLLOW(i),zTLHGH(i)
   10 continue
      close(83)
      RETURN
      END
...
C     selected rows after SELTXR.
     ...
     A LTXRGM(1),LTXRGM(2)
      ...
      K=ITXRCP(jj)
      do 10 ia=1,45
      write(82,'(5E18.9)') ANDVX(ia,K),ADVX(ia,K),AOFX(ia,K),
     A GRLOSX(ia,K),TGAINX(ia,K)
   10 continue
   20 continue
      close(82)
      RETURN
...
C     the reliability chain, before the smoothing save block.
     ...
     CizNMODE(20),zTLLOW(20),zTLHGH(20),zEFF(20),NREL,NMMOD
      ...
      open(81,file=tdir(1:ltdir)//'/son.txt',position='append')
      write(81,'(A,I4,I8,I4)') 'SON ',NINT(GMT),NINT(FREQ*1000.),
     A IPFGX/100
      write(81,'(5E18.9)') ANGLE(IFX),ANGLER(IFX),CPROB(IFX),
     A DBLOS(IFX),DBLOSL(IFX),DBLOSU(IFX),DBU(IFX),DELAY(IFX),
     B DBW(IFX),FLOAT(NHP(IFX)),XNYNOIS(IFX),PROBMP(IFX),RELIAB(IFX),
     C SNDB(IFX),SNPR(IFX),SNRLW(IFX),SNRUP(IFX),SPROB(IFX),
     D VHIGH(IFX),RNEFF(IFX),snxx(IFX),gaint(IFX),gainr(IFX),
     E DU_NOIS(IFX),DL_NOIS(IFX),FLOAT(NREL),XMODE,XMODR
      close(81)
      RETURN
      END
C--------------------------------
...
C     after the 7000-10000 km smoothing blend.
     ...
     C SNRLW(13), SNRUP(13), SPROB(13), VHIGH(13), RNEFF(13),MDL(13)
      ...
      XMODE = ICHAR(MODE(IFX)(1:1))*256 + ICHAR(MODE(IFX)(2:2))
      XMODR = ICHAR(MODER(IFX)(1:1))*256 + ICHAR(MODER(IFX)(2:2))
      open(80,file=tdir(1:ltdir)//'/smo.txt',position='append')
      write(80,'(A,I4,I8,I4)') 'SMO ',NINT(GMT),NINT(FREL(IFX)*1000.),
     A ICHAR(MDL(IFX))
      write(80,'(5E18.9)') ANGLE(IFX),ANGLER(IFX),CPROB(IFX),
     A DBLOS(IFX),DBLOSL(IFX),DBLOSU(IFX),DBU(IFX),DELAY(IFX),
     B DBW(IFX),FLOAT(NHP(IFX)),XNYNOIS(IFX),PROBMP(IFX),RELIAB(IFX),
     C SNDB(IFX),SNPR(IFX),SNRLW(IFX),SNRUP(IFX),SPROB(IFX),
     D VHIGH(IFX),RNEFF(IFX),snxx(IFX),gaint(IFX),gainr(IFX),
