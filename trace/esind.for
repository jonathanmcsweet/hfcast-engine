...
C.....FS(3,II) IS THE CRITICAL FREQUENCY UPPER DECILE
  ...
  105 CONTINUE
      ...
      END
C--------------------------------
      SUBROUTINE trace_esind
C     propcore instrumentation: dumps the sporadic-E parameters at every
C     control point, when PROPCORE_TRACE names a directory.
     ...
     2HTR(50,3), FNSQ(50,3)
      ...
      open(92,file=tdir(1:ltdir)//'/esind.txt',position='append')
      write(92,'(A,E18.9,I4)') 'ESI ',GMT,KM
      do 900 II = 1, KM
      write(92,'(A,4E18.9)') 'PT ',FS(1,II),FS(2,II),FS(3,II),HS(II)
900   continue
      close(92)
      RETURN
      END
